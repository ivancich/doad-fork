...
int get_token_regular() {
  ...
  temp_url = malloc(i = strlen(my_url) + 30);
  snprintf(temp_url, i, "%s/auth", my_url);
#if 0
  json_object_object_add(jdom, "name", json_object_new_string("Default"));
  json_object_array_add(jm, json_object_new_string("password"));
  //	json_object_object_add(jx, "name", json_object_new_string("Default"));
...
#endif
  char strbuff[128];
  snprintf(strbuff, sizeof(strbuff), "X-Auth-User: %s", my_user);
  headers = curl_slist_append(headers, strbuff);
  snprintf(strbuff, sizeof(strbuff), "X-Auth-Key: %s", my_password);
  headers = curl_slist_append(headers, strbuff);
  ca = get_curl_handle();
  if (!ca) return 1;
  curl = ca->h;
...
int process()
{
  ...
  if (my_token) {
  } else if (use_keystone) {
    if (r = get_token_keystone()) {
      fprintf(stderr,"get_token_keystone failed\n");
      return r;
    }
  } else if (r = get_token_regular()) {
    fprintf(stderr,"get_token_regular failed\n");
    return r;
  }
  start_threads();
  wait_for_completion();
...
main(int ac, char **av)
{
  ...
  // NB: make this into a command-line option....
  use_keystone = false;
  ...
  while (--ac > 0) {
	...
	Usage:
	  fprintf(stderr,
		  "Usage: doad3 [-u user] [-p pass] [-P proj] [-t #threads] "
		  "[-h hosturl] [-C capath] [-V] [-s size] -b container\n");
	  exit(1);
	} // switch
      } // while (*++ap)
    } else if (!my_url) {
  ...
  if (!my_container) {
    fprintf(stderr,"must specify -b container\n");
    goto Usage;
  }
  if (!my_user)
